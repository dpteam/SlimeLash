...
package ru.dpt.slimelash.world;

import ru.dpt.slimelash.item.ItemDartDimension;
import ru.dpt.slimelash.block.BlockBitchBlock;
...
public class WorldDartDimension extends ElementsSlimeLash.ModElement {
	public static int DIMID = 3;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("slimelash:dartdimension_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldDartDimension(ElementsSlimeLash instance) {
		super(instance, 7);
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemDartDimension().setUnlocalizedName("dartdimension").setRegistryName("dartdimension"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension dartdimension is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("dartdimension", "_dartdimension", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemDartDimension.block, 0, new ModelResourceLocation("slimelash:dartdimension", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag ? BlockBitchBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? BlockBitchBlock.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("dartdimension_portal");
			setRegistryName("dartdimension_portal");
			setLightLevel(0F);
		}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockBitchBlock.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockBitchBlock.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockBitchBlock.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockBitchBlock.block
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		private static final IBlockState STONE = BlockBitchBlock.block.getDefaultState();
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		private Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("slimelash:magmabiome")),};
