...
package ru.dpt.slimelash.world.biome;
...
public class BiomeMagmaBiome extends ElementsSlimeLash.ModElement {
	@GameRegistry.ObjectHolder("slimelash:magmabiome")
	public static final BiomeGenCustom biome = null;
	public BiomeMagmaBiome(ElementsSlimeLash instance) {
		super(instance, 3);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Magma Biome").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-65434).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("magmabiome");
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.MAGMA.getDefaultState().getBlock() || ground == Blocks.COBBLESTONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.MAGMA.getDefaultState().getBlock() || ground2 == Blocks.COBBLESTONE.getDefaultState().getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.FIRE.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.AIR.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.LOG.getStateFromMeta(3));
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(-1, genh, 0), Blocks.FIRE.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(1, genh, 0), Blocks.FIRE.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, -1), Blocks.FIRE.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, 1), Blocks.FIRE.getDefaultState());
								}
							}
		...
		private void addVines(World world, BlockPos pos) {
			this.setBlockAndNotifyAdequately(world, pos, Blocks.FIRE.getDefaultState());
			int i = 5;
			for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && i > 0; --i) {
				this.setBlockAndNotifyAdequately(world, blockpos, Blocks.FIRE.getDefaultState());
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.LOG.getStateFromMeta(3).getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.MAGMA.getDefaultState().getBlock()
